...
In this chapter, I'll describe the available events and how to use them.
...
Every call to `on` will return an `event-id` which can later be used to `remove` the same callback.
