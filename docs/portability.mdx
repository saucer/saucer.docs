...
<Tabs>
    <TabItem value="qt" label="Qt">
        ...
        * There is no way to specify a status code for custom schemes, thus the user specified one in `saucer::response` is ignored
        * Usage of JS `fetch` with custom schemes (and embedded files for that matter) is only possible since Qt 6.6
        * `saucer::response::headers` are only respected from Qt 6.7 onwards
        * `saucer::request::content()` is only available from Qt 6.7 onwards
    ...
    <TabItem value="macos" label="MacOS / WebKit">
        ...
        The `saucer::options::storage_path` may behave unexpected on macOS. 
...
It is possible to pass certain browser flags to the underlying webview through `saucer::options::browser_flags`.  
...
saucer::webview webview{{ .browser_flags = { "enable-javascript=false" } }};
```
