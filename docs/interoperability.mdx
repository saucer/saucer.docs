...
You can specify the amount of threads used in the `saucer::options` taken by the webview constructor.
...
const result = await window.saucer.call('add_ten', [10]);
// > result == 20
```
...
const result = await window.saucer.exposed.add_ten(10);
// > result == 20
```
:::
...
If you want to use a thread pool you should roll your own.
:::
